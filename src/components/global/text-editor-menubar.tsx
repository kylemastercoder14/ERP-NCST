import {
  ...
  const Button = ({
    ...
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`p-2 ${isActive ? "bg-black text-white border border-input rounded-md" : ""}`}
    >
      {children}
    </button>
  );
  ...
  export default function TextEditorMenuBar({
    ...
    const buttons = [
      ...
      {
        icon: <RiListOrdered2 className="size-4" />,
        onClick: () => editor.chain().focus().toggleOrderedList().run(),
        isActive: editor.isActive("orderedList"),
        disabled: !editor.can().chain().focus().toggleOrderedList().run(),
      },
      {
        icon: <AiOutlineUndo className="size-4" />,
        onClick: () => editor.chain().focus().undo().run(),
        isActive: editor.isActive("undo"),
        disabled: !editor.can().chain().focus().undo().run(),
      },
      {
        icon: <AiOutlineRedo className="size-4" />,
